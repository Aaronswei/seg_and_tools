...
import utils.mask_damaging as mask_damaging

from constant import WRONG_LABEL_PADDING_DISTANCE
...
def create_embedding_segmentation_features(features, feature_dimension,
  ...
  with tf.variable_scope('embedding_seg', reuse=reuse):
    for n in range(n_layers):
      features = deeplab_model.split_separable_conv2d(
...
def get_embeddings(images, model_options, embedding_dimension):
  ...
  """
  features, end_points = deeplab_model.extract_features(
      images,
      model_options,
      is_training=False)
  ...
  if model_options.decoder_output_stride is not None:
    ...
    features = deeplab_model.refine_by_decoder(
        features,
        end_points,
...
def get_logits_with_matching(images,
  ...
  if model_options.decoder_output_stride:
    ...
    if model_options.crop_size is None:
      ...
      height, width = model_options.crop_size
    decoder_height = deeplab_model.scale_dimension(height, 1.0 / decoder_output_stride)
    decoder_width = deeplab_model.scale_dimension(width, 1.0 / decoder_output_stride)
    features = deeplab_model.refine_by_decoder(
        features,
        end_points,
        crop_size=[height, width],
  ...
  for n in range(batch_size):
    ...
    for t in range(num_frames_per_video - n_ref_frames):
      ...
      logits_t = deeplab_model.get_branch_logits(
          embedding_seg_features_n_t,
          1,
...
def create_initial_softmax_from_labels(last_frame_labels, reference_labels,
  ...
  if decoder_output_stride is None:
    ...
    h_sub = deeplab_model.scale_dimension(h, 1.0 / decoder_output_stride)
    w_sub = deeplab_model.scale_dimension(w, 1.0 / decoder_output_stride)
